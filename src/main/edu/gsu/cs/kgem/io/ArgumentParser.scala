...
case class Config(readsFile: File = null, k: Int = 50, threshold: Int = 3,
                  consensusFile: File = null, prThr: Double = -1,
                  epsilon: Double = 0.0025, clustering: File = null,
...
object ArgumentParser {
  ...
  def parseArguments(args: Array[String]): Option[Config] = {
    val parser = new OptionParser[Config]("kGEM") {
      ...
      opt[File]('c',"clustering") action {
        (x, c) => c.copy(clustering = x)
      } text "Path to original (unaligned) reads."
